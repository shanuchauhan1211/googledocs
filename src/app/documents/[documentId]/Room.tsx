...
import { getDocUser } from "@/frontendApis/docApi/docApi";
...
export function Room({ children }: { children: ReactNode }) {
  ...
  const { data: users } = useQuery({
    queryKey: ["Docuser"],
    queryFn: () =>
      getDocUser({ collaboratorIds: currentDoc?.collaboratorIds ?? [] }),
    enabled: !!currentDoc,
  });
  ...
  return (
    <LiveblocksProvider
      ...
      resolveUsers={({ userIds }) => {
        return userIds.map(
          (userId) =>
            users?.Info?.find(({ _id }: { _id: string }) => _id === userId) ??
            undefined,
        );
      }}
      resolveMentionSuggestions={({ text }) => {
        let filteredUser = users.Info;
        if (text) {
          filteredUser = users.Info.filter(({ name }: { name: string }) =>
