...
import { deleteDocument, SearchApi, UpdateDocApi } from "@/frontendApis/docApi/docApi";
...
export default function DocumentTable() {
  ...
  const renameMutation = useMutation({
    mutationFn: async ({
      ...
      newTitle: string;
      action: string;
    }) => UpdateDocApi({ title: newTitle }, action, id),
    ...
    <div className="px-24 py-5">
      ...
      <Table className="">
        <TableHeader className="bg-[#F1F3F4] text-[#4b4b4b]">
          <TableRow>
            <TableHead className="border hover:border-t-[#9b9b9b] ">
              Title
            </TableHead>
            <TableHead className="border hover:border-t-[#9b9b9b] ">
              Created At
        ...
        <TableBody className="bg-[#F1F3F4]">
          {docs && docs.length > 0 ? (
            docs?.map((doc, index) => (
              <TableRow
                className={`${deleteMutation?.status === "pending" ? ` opacity-30` : ``}`}
                key={doc._id || index}
              >
                <TableCell
                ...
                <TableCell className="border hover:border-b-[#9b9b9b]">
                  {doc.updatedAt
                    ? format(new Date(doc.updatedAt), "dd MMM yyyy, hh:mm a")
                    : "N/A"}
                </TableCell>
