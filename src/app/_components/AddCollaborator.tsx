...
import { UpdateDocApi } from "@/docApi/docApi";
...
const addCollaboratorMutation = useMutation({
  mutationFn: async ({ id, collaboratorId ,action}:{id:string;collaboratorId:string;action:string}) => UpdateDocApi({collaboratorId:collaboratorId},action,id),
  ...
  AllUser.filter((user) => !currentDoc?.collaboratorIds?.includes(user._id))
    .map((u) => (
      <MenubarItem
        key={u._id}
        onClick={() =>
