...
const registerSchema = z.object({
    name:z.string().max(12,{message:"Name must be between 5-12 characters"}).min(5,{message:"Name must be between 5-12 characters"}),
...
export async function POST(request: NextRequest) {
    try {
        ...
        const otpRecord = await Otps.findOne({ email:loweredEmail, otpStatus: "completed" });
