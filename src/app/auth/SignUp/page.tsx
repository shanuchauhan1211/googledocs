...
import { registerApi, sentOtp, verifyOtp } from "@/authApi/authApi"; 
...
export default function SignUpPage() {
 ...
 // const setUser = useAuthStore((state) => state.setUser);
  ...
  const [details, setDetails] = useState<{ name: string; email: string; password: string } | null>(null);
  ...
  const { status, mutate: sendOtpMutate } = useMutation({
    mutationFn: sentOtp,
    onError:() =>{showToast("error","Failed to Sent Otp")},
    onSuccess: () => {showToast("success","Otp sent Successfully")},
  });
  ...
  return (
    <>
     ...
     <h2 className="text-2xl font-bold mb-4">Sign Up</h2>
        <form className="flex flex-col gap-4 w-full text-sm  max-w-sm" onSubmit={handleSubmit(onSubmit)}>
          <input {...register("name")}  placeholder="Name" className="p-2 border" />
          {errors.name && <p className="text-red-500">{errors.name.message}</p>}
          <input {...register("email")} placeholder="Email" className="p-2 border" />
          {errors.email && <p className="text-red-500">{errors.email.message}</p>}
          <input {...register("password")} type="password" placeholder="Password" className="p-2 border" />
          {errors.password && <p className="text-red-500">{errors.password.message}</p>}
          <Button variant={"default"} type="submit" className="p-2 bg-blue-500 text-white font-bold" disabled={isLoading}>
            {isLoading ? "Processing..." : "Create Account"}
          </Button>
        </form>
        <Button variant={"link"} className="mt-4 text-blue-500" onClick={() => router.push("/auth/SignIn")}>
    ...
    { toggle && <div className="w-screen absolute top-0 right-0 left-0 flex flex-col items-center justify-center h-screen bg-neutral-600/80 " >
