...
const [mail,setMail]=useState('');
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-gradient-to-r from-blue-500 to-purple-600 ">
     ...
     <div className="flex gap-x-5 items-center "> <span className="mb-2 ">Welcome to  <span className="underline underline-offset-4  font-semibold">Meow Doc</span> </span> <Image alt="logo" width={50} height={50} src='/logo.svg'/></div> 
      <h2 className="text-2xl font-bold mb-4">Login</h2>
      <form className="flex flex-col gap-4 w-full max-w-sm" onSubmit={handleSubmit(onSubmit)}>
        <input {...register("email")} placeholder="Email" className="p-2 border text-sm" />
        {errors.email && <p className="text-red-500">{errors.email.message}</p>}
        <input {...register("password")} type="password" placeholder="Password" className="p-2 border text-sm" />
